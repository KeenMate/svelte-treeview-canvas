<script lang="ts">
	...
	import type { CanvasTheme } from '$lib/canvas-theme.js';
	...
	// Sub-team titles for deeper levels
	const SUB_TITLES: Record<string, string[]> = {
		Engineering: ['Tech Lead', 'Senior Dev', 'Developer', 'Junior Dev', 'Intern'],
		Product:     ['Senior Analyst', 'Analyst', 'Associate', 'Coordinator'],
		Design:      ['Lead Designer', 'Designer', 'Junior Designer', 'Design Intern'],
		Marketing:   ['Campaign Lead', 'Specialist', 'Coordinator', 'Assistant'],
		Sales:       ['Team Lead', 'Senior Rep', 'Rep', 'Trainee'],
		Finance:     ['Senior Analyst', 'Analyst', 'Associate', 'Clerk'],
		HR:          ['Senior Recruiter', 'Recruiter', 'Coordinator', 'Assistant'],
		Operations:  ['Team Lead', 'Specialist', 'Analyst', 'Associate'],
		Legal:       ['Senior Paralegal', 'Paralegal', 'Legal Asst', 'Clerk'],
	};

	const SQUAD_NAMES = ['Alpha', 'Beta', 'Gamma', 'Delta', 'Omega', 'Nova', 'Core', 'Edge'];
	const PROJECT_NAMES = ['Atlas', 'Phoenix', 'Horizon', 'Nebula', 'Vortex', 'Prism', 'Beacon', 'Summit'];
	...
	function generateOrgData(): Person[] {
		...
		function addPerson(path: string, parentPath: string, level: number, title: string, dept: string, hasChildren: boolean, locIdx: number, statusIdx: number) {
			const n = nextName(path);
			nodes.push({
				id: id++, path, parentPath, level,
				name: n.full, title, department: dept,
				email: `${n.first.toLowerCase()}.${n.last.toLowerCase()}@acme.com`,
				avatar: n.initials, hasChildren, reports: 0,
				location: LOCATIONS[locIdx % LOCATIONS.length],
				status: STATUSES[statusIdx % STATUSES.length]
			});
		}

		// Level 1: CEO
		addPerson('1', '', 1, 'CEO', 'Executive', true, 0, 0);
		...
		departments.forEach((dept, di) => {
			...
			const subTitles = SUB_TITLES[dept] || SUB_TITLES['Operations'];

			// Level 2: Department head
			addPerson(deptPath, '1', 2, titles[0], dept, true, di, di);

			// Level 3: Directors / Team leads (3-4 per department)
			const directorCount = 3 + (di % 2);
			for (let dri = 0; dri < directorCount; dri++) {
				const dirPath = `${deptPath}.${dri + 1}`;
				const dirTitle = titles[1 + (dri % Math.max(1, titles.length - 2))];

				addPerson(dirPath, deptPath, 3, dirTitle, dept, true, di + dri + 2, di + dri);

				// Level 4: Squad/team leads (2-3 per director)
				const squadCount = 2 + ((di + dri) % 2);
				for (let si = 0; si < squadCount; si++) {
					const squadPath = `${dirPath}.${si + 1}`;
					const squadName = SQUAD_NAMES[(di + dri + si) % SQUAD_NAMES.length];
					const squadTitle = `${squadName} ${subTitles[0]}`;

					addPerson(squadPath, dirPath, 4, squadTitle, dept, true, di + dri + si, di + dri + si + 1);

					// Level 5: Senior members (2-3 per squad)
					const seniorCount = 2 + ((di + dri + si) % 2);
					for (let sni = 0; sni < seniorCount; sni++) {
						const seniorPath = `${squadPath}.${sni + 1}`;
						const projName = PROJECT_NAMES[(di + dri + si + sni) % PROJECT_NAMES.length];

						// Some seniors lead a sub-project with 2-3 ICs under them
						const hasSubProject = (di + dri + si + sni) % 3 !== 0;

						addPerson(seniorPath, squadPath, 5, `${subTitles[1]} (${projName})`, dept, hasSubProject, di + sni, di + dri + sni);

						if (hasSubProject) {
							// Level 6: ICs on the sub-project (2-3)
							const icCount = 2 + ((di + si + sni) % 2);
							for (let ici = 0; ici < icCount; ici++) {
								const icPath = `${seniorPath}.${ici + 1}`;
								const icTitleIdx = Math.min(2 + ici, subTitles.length - 1);

								// Some ICs mentor interns (level 7)
								const hasIntern = ici === 0 && (di + dri) % 3 === 0;

								addPerson(icPath, seniorPath, 6, subTitles[icTitleIdx], dept, hasIntern, di + ici + si, di + dri + ici);

								if (hasIntern) {
									// Level 7: Intern
									const internPath = `${icPath}.1`;
									const internTitle = subTitles[subTitles.length - 1];
									addPerson(internPath, icPath, 7, internTitle, dept, false, di + ici, di + si + ici);
								}
							}
						}
					}
